...
public final class ExoMediaSourceHelper {
    ...
    public MediaSource getMediaSource(String uri, Map<String, String> headers, boolean isCache, int contentType) {
        ...
        if (contentType == PlaybackException.ERROR_CODE_PARSING_CONTAINER_UNSUPPORTED) {
            MediaItem.Builder builder = new MediaItem.Builder().setUri(uri);
            builder.setMimeType(MimeTypes.APPLICATION_M3U8);
            return new DefaultMediaSourceFactory(getDataSourceFactory(), getExtractorsFactory()).createMediaSource(getMediaItem(uri, contentType));
        }
        switch (contentType) {
            case C.TYPE_DASH:
